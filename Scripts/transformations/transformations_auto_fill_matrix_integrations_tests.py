...
def _make_matrix(table_selected=0):
    ...
    import transformations.auto_fill.auto_fill_matrix as autofill
    ...
    query = m[0][0].AttachedToQuery
    
    
    fill = autofill.FillMatrix(m, conn, query)
...
class Test(TestCase):
    ...
    def test_auto_fill_rows_default(self):
        m,fill = _make_matrix()
        m[0][0].RowSelection = "0"
        m[0][0].ColumnSelection = "0"
